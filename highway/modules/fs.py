...
class ClfcImgReader(Node):
    ...
    def __init__(self, data_dir, batch_size, shape, file_map=None):
        ...
        super(ClfcImgReader, self).__init__()

    def run(self):
        if not self.file_map:
            self.file_map, self.n_classes, self.classes = get_class_file_map(self.data_dir)
...
class ImgReader(Node):
    ...
    def run(self):
        filenames = get_directory_filenames(self.data_dir, ImgReader.FILETYPES)
...
class JpgSaver(StreamWriter):
    ...
    def dump_func(self, stream):
        ...
        for item, key in zip(batch, keys):
            imsave(self.out_dir + "/" + str(key) + ".jpg", item)